...
import { postData, postFormData, sendGMessageToServer, sendMessageToServer } from './API/ApiCalls';
import { ApiUrls } from './API/ApiUrl';
import { insert } from './API/DManager';
import Color from './assets/Color/Color';
import { openDatabase } from 'react-native-sqlite-storage';
import { encryptMyData } from './EncrypDecrypt';
var db = openDatabase('Khan.db');

const audioRecorderPlayer = new AudioRecorderPlayer();
const GroupRecorder = ({route}) => {
  ...
  async function sendAudioMessage(uri) {
    // console.log(uri.slice(uri.indexOf("files0")));
    create_group('Group'+user.Phone);
    



    let name = uri.slice(uri.indexOf('files0') + 7);
    let type = "aduio/mp3";

    const eFileName=await encryptMyData(name);
    ...
    //   Friend_ID: route.params.Friend_ID,
      ...
      Message_Type: 'audio',
      Message_Content:JSON.stringify(eFileName),//base64String,
      Is_Download: 0,
    ...
    body.append('To_ID',msgInfo.To_ID);
    body.append('Sent_By',msgInfo.Sent_By);
    ...
    // const resp = await postFormData(`${ApiUrls.Message._uploadFile}`, body);
    const resp = await postFormData(`${ApiUrls.Group._uploadFile}`, body);

    console.log('res of recording in group : ', resp);

    // const base64String = await RNFS.readFile(uri, "base64");
    if (resp.status === 200) {
      insert('Group' + user.Phone, 'From_ID,To_ID,Sent_By,Message_Content,Message_Type,Is_Seen,Is_Download, Created_Date, Created_Time', [user.Phone, G_ID, user.Name ,uri, 'audio', 1, 1, msgInfo.Created_Date, msgInfo.Created_Time], '?,?,?,?,?,?,?,?,?');
