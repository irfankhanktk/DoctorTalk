...
export const create_User_Table = async (Phone) => {
    ...
    db.transaction((tx) => {
        ...
        tx.executeSql(
            'CREATE TABLE IF NOT EXISTS User'+ Phone +'(User_ID INTEGER PRIMARY KEY, Image TEXT, Name TEXT, Phone TEXT, Role TEXT,Friend_Type TEXT)',
            [],
...
export const deleteCCDRecords = async (Phone) => {

    await db.transaction((tx) => {
        tx.executeSql('DELETE FROM CCD WHERE Patient_ID=?', [Phone],
            (tx, results) => console.log('successfully Removed CCD Records '),
            (tx, results) => console.log('failed to Remove CCD Records ')
        );
    });

}
export const create_group=async (tableName)=>{
    db.transaction((tx) => {
        // tx.executeSql('DROP TABLE IF EXISTS ' + tableName, [],
        //     (tx, results) => alert('successfully droped ' + tableName),
        //     (tx, results) => alert('failed to drop table ' + tableName)
        // );
        tx.executeSql(
            'CREATE TABLE IF NOT EXISTS ' + tableName + '(GM_ID INTEGER PRIMARY KEY AUTOINCREMENT, From_ID TEXT, To_ID TEXT, Sent_By TEXT, Message_Content Text,Message_Type TEXT,Is_Seen INTEGER,Is_Download INTEGER,Created_Date TEXT, Created_Time TEXT)',
            [],
            (tx, results) => { console.log('successfully inserted ' + tableName) },
            (tx, results) => { console.log('failed to create table ' + tableName) }
        );
    });
}