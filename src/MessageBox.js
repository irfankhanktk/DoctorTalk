...
import { getData, postData, postFormData, sendMessageToServer } from './API/ApiCalls';
...
const MessageBox = ({ route }) => {
    ...
    const sendImage = async (imageData) => {
        ...
        // }
        //using formdata to send message
    ...
    console.log('msg :',msgInfo);
        ...
        body.append('To_ID',route.params.Phone);
        ...
        if (resp.status === 200) {
            sendMessageToServer(socket, msgInfo);
            insert('Message' + route.params.Friend_ID, 'From_ID,To_ID,Message_Content,Message_Type,Is_Seen,Is_Download,Created_Date,Created_Time', [user.Phone, route.params.Phone, imageData.uri, msgInfo.Message_Type, 1, 1, msgInfo.Created_Date, msgInfo.Created_Time], '?,?,?,?,?,?,?,?');
            select();
        } else {
            alert('Something went wrong');
