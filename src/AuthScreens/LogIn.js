...
const LogIn = ({ navigation }) => {
    ...
    const setData = async () => {
        ...
        if (isRememberMe) {
            await AsyncStorage.setItem('RememberMe', userData.Phone);
    ...
    const onSignUp = async () => {
        ...
        if (!pattern.test(contact)) {
            ...
            if (isPatient) {
                ...
                if (res.status === 200 && res.data === "Already Account") {
    ...
    const getUserData = async () => {
        const phone = await AsyncStorage.getItem("RememberMe");
    ...
    return (
        <ScrollView style={styles.container}>
            ...
            <View style={styles.footer}>
                <CustomInputText value={contact} setOnChangeText={(t) => setContact(t)} iconName='phone' placeholder='Mobile Number' />
                {isSignUp && (
                    <>
